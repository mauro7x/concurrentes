...
use crate::{paths::{BANK_ACCOUNTS}, types::{common::BoxResult, data::{Action, Transaction}}};
...
impl Bank {
    ...
    pub fn reserve_resources(&mut self, tx: &Transaction) -> BoxResult<Action> {
        ...
        if *account_money < required_money {
            println!("[tx {}] insufficient money - required: {} available: {}", tx.id, required_money, *account_money);
            // TODO: Reject transaction
        }

        let account_left_money = *account_money - required_money;
        println!("[tx {}] taking {} from cbu {} - money left: {}", tx.id, required_money, tx.cbu, account_left_money);
    ...
    pub fn release_resources(&mut self, tx: &Transaction) -> BoxResult<()> {
        let account_money = self.accounts_money
            .get_mut(&tx.cbu)
            .ok_or(format!("[tx {}] ERROR: account with cbu {} should have been created", tx.id, tx.cbu))?;
        ...
        *account_money += money_to_release;
        println!("[tx {}] releasing {} to cbu {} - money available: {}", tx.id, money_to_release, tx.cbu, account_money);
    ...
    pub fn consume_resources(&mut self, tx: &Transaction) -> BoxResult<()> {
        ...
        println!("[tx {}] confirming extraction of {} from cbu {}", tx.id, tx.airline_cost + tx.hotel_cost, tx.cbu);
        Ok(())
    }
}
