...
impl Service {
    ...
    fn respond_message(&mut self, alglobo: &String, req: &Message, action: Action) -> Result<(), std::io::Error> {
        let res = Message {
            from: self.name,
            action,
            tx: req.tx
    ...
    fn process_txs(&mut self) -> Result<(), std::io::Error> {
        let (src, req) = match recv_msg(&mut self.conn) {
            Ok((src, req)) => (src, req),
            Err(err) => panic!("{:#?}", err)
        };
        ...
        match (self.tx_log.get(&req.tx), req.action) {
            ...
            (Some(logged_action), new_action) if (*logged_action == new_action) => {
                println!("[tx {}] resending already processed response for action {:?}", req.tx, req.action);
